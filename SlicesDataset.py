...
class SlicesDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # <YOUR CODE GOES HERE>
        #sample["image"] = torch.from_numpy(self.data[slc[0]]['image'][slc[1]][None, :]).unsqueeze(0).type(torch.FloatTensor)
        #sample["seg"] = torch.from_numpy(self.data[slc[0]]['seg'][slc[1]][None, :]).unsqueeze(0)
        img_s = self.data[slc[0]]['image'][None, slc[1]]
        slc_image_tensor = torch.from_numpy(img_s)
        
        seg_s = self.data[slc[0]]['seg'][None, slc[1]]       
        slc_seg_tensor = torch.from_numpy(seg_s)
        
        sample.update({"image": slc_image_tensor, "seg": slc_seg_tensor})
        return sample
